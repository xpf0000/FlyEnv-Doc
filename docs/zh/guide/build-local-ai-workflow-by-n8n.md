...
FlyEnv 将 n8n 和 Ollama 作为本地进程运行，避免 Docker Desktop 的基础虚拟机开销。实际资源占用取决于模型大小、工作流和启用的服务。
...
**通常可以，但取决于工作负载。** FlyEnv 直接在系统上运行 n8n 和 Ollama，避免容器虚拟机的基础开销；模型大小、活动工作流、数据库和容器配置都会影响实际差异。
