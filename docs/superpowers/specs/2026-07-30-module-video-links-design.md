...
- Convert the five affected cards in `docs/components/AppModules/index.vue` into the existing English video-card pattern, with the supplied YouTube URLs and English video titles.
- Convert the same five cards in `docs/components/AppModules/zh.vue` into the existing Chinese video-card pattern, with the supplied Bilibili URLs and Chinese video titles.
