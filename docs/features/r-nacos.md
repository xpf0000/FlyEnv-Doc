...
FlyEnv manages the local R-Nacos runtime, its env file and its data directory; it does not guarantee that every R-Nacos version is available on every operating system or install source. Homebrew installs are offered on macOS and Linux, static builds across platforms, and the MacPorts source is not enabled for this module. Note that the console button determines the port to open independently of the `rnacos.env` file, so if you change the console port away from 10848, open the console manually at your configured address. For what can actually be installed on your machine, rely on the in-app version list and the [Download page](/download).
