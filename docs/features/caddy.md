...
Caddy is one of the web servers that can serve the sites you create in FlyEnv's Host module. Only PHP-type sites get a generated Caddy vhost; Node, Java, Go and Python sites are reached through reverse proxy, and Tomcat sites live in `server.xml`.
