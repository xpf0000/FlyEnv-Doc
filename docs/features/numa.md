...
Numa is available on macOS, Linux and Windows. On Linux the service needs elevated privileges to bind port 53; on macOS and Windows no special setup is required beyond the install itself. MacPorts is not offered as an install source for this module — use the static list or Homebrew instead. The configuration editor is raw TOML only, and the web UI is Numa's own interface, so its features and layout follow the upstream project rather than FlyEnv. To see which Numa versions your platform can install, check the Version Manager in the app after getting FlyEnv from the [Download page](/download); hands-on walkthroughs are collected on the [Demos page](/demos).
